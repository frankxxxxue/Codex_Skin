...
> macOS 暂不提供单文件版（Node SEA 在 Intel Mac 上会段错误），请用方式二的双击脚本。
...
2. 双击对应平台的脚本：
   - Windows：`启动皮肤.bat`
   - macOS：`启动皮肤.command`（从 Git 下载后首次需先 `chmod +x 启动皮肤.command`，再右键 → 打开）
...
- macOS 下通过 Spotlight 自动发现 ChatGPT 位置（不依赖安装路径）；若自动发现失败，用 `--exe` 手动指定。
